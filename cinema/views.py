...
from .models import (
    ...
    Review, Promotion, Rule, User, Genre, PasswordResetToken
)
from .forms import (
        ...
        if form.is_valid():
            ...
            try:
                ...
                reset_url = request.build_absolute_uri(
                    reverse('cinema:password_reset_confirm') + f'?token={token}'
                )
                
                subject = 'Восстановление пароля - КиноМир'
                message = f'''
Здравствуйте, {user.username}!

Вы запросили восстановление пароля для вашего аккаунта на сайте КиноМир.
...
Команда КиноМир
                ...
                try:
                    ...
                    # В режиме заглушки (console backend) письмо выводится в консоль сервера
                    # Для тестирования можно посмотреть логи сервера
                    if settings.EMAIL_BACKEND == 'django.core.mail.backends.console.EmailBackend':
                        messages.success(
                            request,
                            f'Инструкции по восстановлению пароля отправлены на ваш email. '
                            f'В режиме разработки письмо выводится в консоль сервера.'
                        )
                    else:
                        messages.success(
                            request,
                    ...
                    messages.error(
                        request,
                        f'Ошибка отправки письма: {str(e)}. Обратитесь в поддержку.'
                    )
                ...
                messages.success(
                    request,
                    'Если пользователь с таким email существует, инструкции отправлены на email.'
                )
            except Exception as e:
    ...
    return render(request, 'cinema/password_reset_request.html', context)
...
def password_reset_confirm(request):
    ...
    if not token:
        messages.error(request, 'Неверная ссылка для восстановления пароля.')
        return redirect('cinema:password_reset_request')
    ...
    try:
        ...
        return render(request, 'cinema/password_reset_confirm.html', context)
        
    except PasswordResetToken.DoesNotExist:
        messages.error(request, 'Неверная или уже использованная ссылка для восстановления пароля.')
        return redirect('cinema:password_reset_request')
    except Exception as e:
        messages.error(request, f'Произошла ошибка: {str(e)}')
        return redirect('cinema:password_reset_request')
